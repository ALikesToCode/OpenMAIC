...
export function getAvailableProvidersWithVoices(
  ...
  for (const [id, config] of Object.entries(TTS_PROVIDERS)) {
    ...
    if (hasApiKey || isServerConfigured || isLocalVoxCPM || isKeylessLocalProvider) {
      ...
      // Build model groups
      let modelGroups: ModelVoiceGroup[] = [];
      if (config.models.length > 0) {
        modelGroups = getTTSModelVoiceGroups(providerId, {
          ...
          voices: group.voices.map((voice) => ({
            id: voice.id,
            name: voice.name,
            language: config.voices.find((candidate) => candidate.id === voice.id)?.language,
          })),
        }));
  ...
  for (const [id, providerConfig] of Object.entries(ttsProvidersConfig)) {
    ...
    if (
      ...
      providerConfig.isServerConfigured !== true
    ) {
      continue;
    }
    if (
