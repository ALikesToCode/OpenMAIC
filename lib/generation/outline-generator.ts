...
import { buildSceneCountGuidance, formatExistingOutlinesForPrompt } from './scene-count-guidance';
...
export async function generateSceneOutlinesFromRequirements(
  ...
  const sceneCountGuidance = buildSceneCountGuidance({
    ...
    researchContextLength: options?.researchContext?.trim().length ?? 0,
    existingSceneCount: options?.existingOutlines?.length ?? 0,
    sceneCountTarget: options?.sceneCountTarget,
