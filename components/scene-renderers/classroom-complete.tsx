...
const DEFAULT_EXTENSION_SCENE_COUNT = 3;
...
export function ClassroomCompletePage({
  ...
  const prefersReducedMotion = useReducedMotion();
  const [customSceneCount, setCustomSceneCount] = useState(DEFAULT_EXTENSION_SCENE_COUNT);
        ...
        <div className="relative flex flex-col items-center gap-6 max-w-2xl w-full px-8 py-10">
          ...
          {onGenerateMoreScenes && (
            <motion.div
              ...
              className="w-full rounded-2xl bg-white/90 dark:bg-gray-900/70 border border-amber-100 dark:border-amber-900/40 px-5 py-4 shadow-sm"
            >
              <div>
              ...
              <div className="mt-3 flex flex-col sm:flex-row gap-2">
                <button
                  type="button"
                  onClick={() => {
                    void onGenerateMoreScenes(DEFAULT_EXTENSION_SCENE_COUNT);
                  }}
                  disabled={isGeneratingMoreScenes}
                  className="h-9 px-4 rounded-lg bg-amber-500 text-white text-xs font-bold shadow-sm shadow-amber-500/20 hover:bg-amber-600 disabled:opacity-60 disabled:cursor-wait transition-colors"
                >
                  {isGeneratingMoreScenes
                    ? t('stage.generatingMoreScenes')
                    : t('stage.extendClassroomAuto', { count: DEFAULT_EXTENSION_SCENE_COUNT })}
                </button>
                  ...
                  <input
                    ...
                    onChange={(event) => {
                      ...
                      if (Number.isFinite(value)) {
                        setCustomSceneCount(Math.min(20, Math.max(1, Math.round(value))));
                      }
                    }}
                    disabled={isGeneratingMoreScenes}
                    aria-label={t('stage.extendClassroomCustom')}
                    className="h-9 w-16 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-950 px-2 text-center text-sm font-semibold text-gray-900 dark:text-gray-100 disabled:opacity-60"
                  />
                  <button
                    type="button"
                    onClick={() => {
                      void onGenerateMoreScenes(customSceneCount);
                    }}
                    disabled={isGeneratingMoreScenes}
                    className="h-9 px-4 rounded-lg border border-amber-200 dark:border-amber-800 text-xs font-bold text-amber-700 dark:text-amber-300 hover:bg-amber-50 dark:hover:bg-amber-950/30 disabled:opacity-60 disabled:cursor-wait transition-colors"
                  >
...
export function ClassroomCompletePageConnected({
  onGenerateMoreScenes,
  isGeneratingMoreScenes,
}: Pick<ClassroomCompletePageProps, 'onGenerateMoreScenes' | 'isGeneratingMoreScenes'>) {
