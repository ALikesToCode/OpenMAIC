...
import { normalizeOutlineLanguage, type SceneLanguage } from '@/lib/generation/outline-language';
...
type SceneOutlinesStreamRequestBody = {
...
export async function POST(req: NextRequest) {
  ...
  try {
    const body = (await req.json()) as SceneOutlinesStreamRequestBody;
    ...
    const sceneCountGuidance = buildSceneCountGuidance({
      requirement: requirements.requirement,
      pdfTextLength: pdfText?.trim().length ?? 0,
      researchContextLength: researchContext?.trim().length ?? 0,
      existingSceneCount: body.existingOutlines?.length ?? 0,
      sceneCountTarget: body.sceneCountTarget,
      additionalSceneCountTarget:
        body.additionalSceneCountTarget ?? body.extensionRequest?.additionalSceneCountTarget,
    });
    const requestedLanguage =
      requirements.language === 'en-US' || requirements.language === 'zh-CN'
        ? (requirements.language as SceneLanguage)
        : undefined;
    ...
    const prompts = buildPrompt(promptId, {
      requirement: requirements.requirement,
      pdfContent: promptPdfContent || 'None',
      availableImages: availableImagesText,
      researchContext: researchContext || 'None',
      sceneCountGuidance,
      existingCourseContext: formatExistingOutlinesForPrompt(body.existingOutlines),
      hasSourceImages,
            ...
            try {
              ...
              for await (const chunk of textStream) {
                ...
                for (const outline of newOutlines) {
                  ...
                  const normalized = normalizeOutlineLanguage(enriched, requestedLanguage);
                  parsedOutlines.push(normalized);
